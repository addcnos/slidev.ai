...
# 部署 Slidev AI 站点 {#deploy-your-slidev-ai-site}

以下指南基于一些前提：

- Slidev AI 站点位于项目的 `slides` 目录中。
- 你使用的是默认的生成输出目录 (`.slidev/dist`)。
- Slidev AI 作为本地依赖项安装在项目中，并且你已在 `package.json` 中设置以下脚本：

  ```json
  {
    "scripts": {
      "slides:build": "slidev build slides",
      "slides:preview": "slidev preview slides"
    }
  }
  ```
...
2. 构建文档后，通过运行以下命令可以在本地预览它：
   ...
   `preview` 命令将启动一个本地静态 Web 服务 `http://localhost:5000`，该服务以 `.slidev/dist` 作为源文件。这是检查生产版本在本地环境中是否正常的一种简单方法。
...
3. 可以通过传递 `--port` 作为参数来配置服务器的端口。
   ...
   {
     "scripts": {
       "slides:preview": "slidev preview slides --port 8080"
     }
   }
   ```

   现在 `slides:preview` 方法将会在 `http://localhost:8080` 启动服务。

## 设定 public 根目录 {#setting-a-public-base-path}

默认情况下，我们假设站点将部署在域名 (`/`) 的根路径上。如果站点在子路径中提供服务，例如 `https://mywebsite.com/presentation/`，则需要在 Slidev AI 配置中将 `base` 选项设置为 `'/presentation/'`。
...
注意：该 `_headers` 文件应放置在 `public` 目录中 (在我们的例子中是 `slides/public/_headers`)，以便将其逐字复制到输出目录。
...
- **构建命令：** `npm run slides:build`
- **输出目录：** `slides/.slidev/dist`
- **node 版本：** `18` (或更高版本)
...
1. 在项目的 `.github/workflows` 目录中创建一个名为 `deploy.yml` 的文件，其中包含这样的内容：
   ...
   ```yaml
   # 构建 Slidev AI 站点并将其部署到 GitHub Pages 的示例工作流程
   #
   name: Deploy Slidev AI site to Pages
   ...
   jobs:
     ...
     build:
       ...
       steps:
         ...
         - name: Build with Slidev AI
           run: npm run slides:build # 或 pnpm slides:build / yarn slides:build / bun run slides:build
         - name: Upload artifact
           uses: actions/upload-pages-artifact@v3
           with:
             path: slides/.slidev/dist
   ...
   ::: warning
   确保 Slidev AI 中的 `base` 选项配置正确。有关更多详细信息，请参阅[设置根路径](#setting-a-public-base-path)。
   :::
...
1. 如果你想部署到 `https://<username>.gitlab.io/<repository>/`，将 Slidev AI 配置中的 `outDir` 设置为 `../public`。将 `base` 选项配置为 `'/<repository>/'`。
...
2. 在配置文件中设置这些值 (并删除不需要的值，如 `api_location`)：
   ...
   - **`app_location`**: `/`
   - **`output_location`**: `slides/.slidev/dist`
   - **`app_build_command`**: `npm run slides:build`
...
2. 运行 `npm run slides:build` 后，运行此命令进行部署：
...
1. 运行 `npm run slides:build` 后，运行此命令进行部署：

   ```sh
   npx surge slides/.slidev/dist
   ```
...
2. 使用以下内容在项目的根目录中创建一个名为 `static.json` 的文件：
   ...
   ```json
   {
     "root": "slides/.slidev/dist"
   }
   ```

### Edgio

请参阅[创建并部署 Slidev AI 应用程序到 Edgio](https://docs.edg.io/guides/vitepress)。

### Kinsta 静态站点托管 {#kinsta-static-site-hosting}

你可以按照这些[说明](https://kinsta.com/docs/vitepress-static-site-example/) 在 [Kinsta](https://kinsta.com/static-site-hosting/) 上部署 Slidev AI 站点。

### Stormkit

你可以按照这些[说明](https://stormkit.io/blog/how-to-deploy-vitepress)将你的 Slidev AI 项目部署到 [Stormkit](https://www.stormkit.io)。

### Nginx

下面是一个 Nginx 服务器块配置示例。此配置包括对基于文本的常见资源的 gzip 压缩、使用适当缓存头为 Slidev AI 站点静态文件提供服务的规则以及处理 `cleanUrls: true` 的方法。
...
本配置默认已构建的 Slidev AI 站点位于服务器上的 `/app` 目录中。如果站点文件位于其他位置，请相应调整 `root` 指令。
